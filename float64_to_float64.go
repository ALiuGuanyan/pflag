...
func (f *FlagSet) Float64ToFloat64Var(p *map[float64]float64, name string, value map[float64]float64, usage string) {
	f.VarP(newFloat64ToFloat64Value(value, p), name, "", usage)
}
...
func (f *FlagSet) Float64ToFloat64VarP(p *map[float64]float64, name, shorthand string, value map[float64]float64, usage string) {
	f.VarP(newFloat64ToFloat64Value(value, p), name, shorthand, usage)
}

// Float64ToFloat64Var defines a string flag with specified name, default value, and usage string.
// The argument p points to a map[float64]float64 variable in which to store the value of the flag.
// The value of each argument will not try to be separated by comma
func Float64ToFloat64Var(p *map[float64]float64, name string, value map[float64]float64, usage string) {
	CommandLine.VarP(newFloat64ToFloat64Value(value, p), name, "", usage)
}

// Float64ToFloat64VarP is like Float64ToFloat64Var, but accepts a shorthand letter that can be used after a single dash.
func Float64ToFloat64VarP(p *map[float64]float64, name, shorthand string, value map[float64]float64, usage string) {
	CommandLine.VarP(newFloat64ToFloat64Value(value, p), name, shorthand, usage)
}
...
// The value of each argument will not try to be separated by comma
func Float64ToFloat64(name string, value map[float64]float64, usage string) *map[float64]float64 {
	return CommandLine.Float64ToFloat64P(name, "", value, usage)
}

// Float64ToFloat64P is like Float64ToFloat64, but accepts a shorthand letter that can be used after a single dash.
func Float64ToFloat64P(name, shorthand string, value map[float64]float64, usage string) *map[float64]float64 {
	return CommandLine.Float64ToFloat64P(name, shorthand, value, usage)
}
